...
export default function ApiKeyModal({ open, accountName, apiKey, repos = [], onClose }: ApiKeyModalProps) {
  ...
  return (
    // The key is shown once: Escape / backdrop only close after it is acknowledged.
    <Dialog
      open={open}
      onClose={handleClose}
      labelledBy={titleId}
      dismissible={acknowledged}
      className="bg-surface-2 rounded-lg shadow-xl w-full max-w-[calc(100vw-2rem)] sm:max-w-lg mx-4 flex flex-col max-h-[90vh] outline-none"
    >
