...
export function TeamSwitcher({ teams, currentTeamId }: { teams: Team[]; currentTeamId: string | null }) {
  ...
  if (teams.length <= 1) {
    return (
      <span className="hidden sm:inline text-sm text-text-secondary truncate max-w-[140px]">
        {currentTeam.name}
      </span>
    );
  }
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <span className="hidden sm:inline truncate max-w-[140px]">{currentTeam.name}</span>
        <svg
