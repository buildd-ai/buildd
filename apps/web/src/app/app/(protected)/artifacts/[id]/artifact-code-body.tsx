...
const RAW_DIFF_START = /^(diff --git |Index: |--- |\+\+\+ |@@ )/;
...
const PRE_CLS =
  'max-w-full overflow-x-auto text-sm font-mono text-text-secondary whitespace-pre-wrap [overflow-wrap:anywhere] md:whitespace-pre md:[overflow-wrap:normal]';

/**
 * Body of a `data` or `diff` artifact. Writers often file a markdown summary
 * of a change as a `diff`; that renders as markdown. A raw diff or JSON stays
 * monospace.
 */
export function ArtifactCodeBody({ type, content }: { type: 'data' | 'diff'; content: string }) {
  ...
  return <pre className={PRE_CLS}>{text}</pre>;
}
