...
describe('MissionReleaseSection — none archetype (AC-40)', () => {
  it('renders nothing when data is null', () => {
    const html = renderToStaticMarkup(
      <MissionReleaseSection data={null} workspaceId="ws-1" releaseNowState={releaseNowEnabled} />,
    );
    expect(html).toBe('');
  });
});
...
describe('MissionReleaseSection — gated archetype (AC-40, AC-46)', () => {
  it('renders nothing when the queue is genuinely clean (zero unshipped)', () => {
    const html = renderToStaticMarkup(
      <MissionReleaseSection data={gated({ queueDepth: val(0) })} workspaceId="ws-1" releaseNowState={releaseNowEnabled} />,
    );
    expect(html).toBe('');
  });
  ...
  it('renders nothing when queueDepth is unavailable (no baseline at all)', () => {
    const html = renderToStaticMarkup(
      <MissionReleaseSection data={gated({ queueDepth: unavailable as any })} workspaceId="ws-1" releaseNowState={releaseNowEnabled} />,
    );
    expect(html).toBe('');
  });
  ...
  it('4 merged tasks, no release yet → shows queue depth 4, oldest age, and a link to the release', () => {
    const html = renderToStaticMarkup(
      <MissionReleaseSection
        data={gated({ queueDepth: val(4), baselineSource: 'prod_head', releaseId: 'rel-9' })}
        workspaceId="ws-1"
  ...
  it('carries the Release now trigger action, enabled when release strategy is not branch_merge and Vercel token present', () => {
    const html = renderToStaticMarkup(
      <MissionReleaseSection data={gated({})} workspaceId="ws-1" releaseNowState={releaseNowEnabled} />,
    );
  ...
  it('Release now is present but disabled with a tooltip when the Vercel token is missing (AC-46)', () => {
    ...
    const html = renderToStaticMarkup(
      <MissionReleaseSection data={gated({})} workspaceId="ws-1" releaseNowState={state} />,
    );
    expect(html).toContain('Release now');
...
describe('MissionReleaseSection — continuous archetype', () => {
  it('renders nothing when there is no deploy state yet', () => {
    const html = renderToStaticMarkup(
      <MissionReleaseSection data={continuous({ state: null })} workspaceId="ws-1" releaseNowState={releaseNowEnabled} />,
    );
    expect(html).toBe('');
  });
  ...
  it('shows last deploy state and healthy-since, and the trigger', () => {
    const html = renderToStaticMarkup(
      <MissionReleaseSection data={continuous({ state: 'healthy' })} workspaceId="ws-1" releaseNowState={releaseNowEnabled} />,
    );
