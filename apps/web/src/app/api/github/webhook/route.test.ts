...
describe('POST /api/github/webhook', () => {
  ...
  describe('pull_request auto-merge for repos without CI', () => {
    ...
    it('completes task — branch_merge workspace: Path A handles release, Path B does NOT dispatch', async () => {
      ...
      // branch_merge workspace — Path B must skip (Path A is authoritative)
      mockWorkspacesFindFirst.mockReturnValue({
        id: 'ws1',
      ...
      // Path B must NOT dispatch any workflow for branch_merge workspaces
    ...
    it('workflow_dispatch + every_merge: dispatches the configured workflowFile (not hardcoded release.yml)', async () => {
      const payload = {
        action: 'closed',
        pull_request: {
          number: 8,
