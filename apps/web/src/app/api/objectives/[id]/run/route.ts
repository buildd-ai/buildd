 ...
 *
 * Manually trigger an immediate planning task for an objective.
  ...
  try {
    ...
    if (!objective.workspaceId) {
      return NextResponse.json(
        { error: 'Objective must have a workspace to create tasks' },
        { status: 400 }
      );
    }
    ...
    if (objective.status !== 'active') {
      return NextResponse.json(
        { error: `Cannot run objective with status: ${objective.status}. Only active objectives can be run.` },
        { status: 400 }
      );
    }
    ...
    const taskTitle = `Objective: ${objective.title}`;
    ...
    console.error('Run objective error:', error);
    return NextResponse.json({ error: 'Failed to run objective' }, { status: 500 });
  }
}
