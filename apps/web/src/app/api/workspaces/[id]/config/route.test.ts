...
describe('POST /api/workspaces/[id]/config', () => {
  ...
  it('saves skillInstallerAllowlist when provided', async () => {
    mockGetCurrentUser.mockResolvedValue({ id: 'user-1' });
    mockWorkspacesFindFirst.mockResolvedValue({ id: 'ws-1' });
    ...
    const req = new NextRequest('http://localhost:3000/api/workspaces/ws-1/config', {
      ...
      body: JSON.stringify({
        defaultBranch: 'main',
        skillInstallerAllowlist: ['buildd skill install', 'npm install @buildd/'],
      }),
    });
    const res = await POST(req, { params: mockParams });
    ...
    expect(res.status).toBe(200);
    const data = await res.json();
    expect(data.gitConfig.skillInstallerAllowlist).toEqual([
      'buildd skill install',
      'npm install @buildd/',
    ]);
  });

  it('filters empty strings from skillInstallerAllowlist', async () => {
    mockGetCurrentUser.mockResolvedValue({ id: 'user-1' });
    mockWorkspacesFindFirst.mockResolvedValue({ id: 'ws-1' });
    ...
    const req = new NextRequest('http://localhost:3000/api/workspaces/ws-1/config', {
      ...
      headers: new Headers({ 'content-type': 'application/json' }),
      body: JSON.stringify({
        skillInstallerAllowlist: ['buildd skill install', '', '  ', 123],
      }),
    });
    const res = await POST(req, { params: mockParams });
    ...
    expect(res.status).toBe(200);
    const data = await res.json();
    expect(data.gitConfig.skillInstallerAllowlist).toEqual(['buildd skill install']);
  });

  it('omits skillInstallerAllowlist when not an array', async () => {
    mockGetCurrentUser.mockResolvedValue({ id: 'user-1' });
    mockWorkspacesFindFirst.mockResolvedValue({ id: 'ws-1' });
    ...
    const req = new NextRequest('http://localhost:3000/api/workspaces/ws-1/config', {
      ...
      headers: new Headers({ 'content-type': 'application/json' }),
      body: JSON.stringify({
        skillInstallerAllowlist: 'not-an-array',
      }),
    });
    const res = await POST(req, { params: mockParams });
    ...
    expect(res.status).toBe(200);
    const data = await res.json();
    expect(data.gitConfig.skillInstallerAllowlist).toBeUndefined();
  });
});
