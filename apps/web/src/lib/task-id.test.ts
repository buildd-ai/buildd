...
describe('isValidTaskId', () => {
  describe('valid UUIDs', () => {
    ...
    it('accepts a UUID with all-hex segments', () => {
      expect(isValidTaskId('46e91502-dead-beef-cafe-123456789abc')).toBe(true);
    });
  });

  describe('zero-padded regression pattern', () => {
    it('rejects the production regression pattern (real prefix + zero-padded suffix)', () => {
      // This is the exact mangled form from the Vercel 404 logs:
      // real ID bf442fcb-6179-43b3-aa92-2564b1ad24b8 → bf442fcb-0000-0000-0000-000000000000
      expect(isValidTaskId('bf442fcb-0000-0000-0000-000000000000')).toBe(false);
    });

    it('rejects a zero-padded ID with a different real prefix', () => {
      expect(isValidTaskId('08e2db98-0000-0000-0000-000000000000')).toBe(false);
    });
  });

  describe('malformed IDs', () => {
    it('rejects an 8-char short ID (pre-dash prefix only)', () => {
