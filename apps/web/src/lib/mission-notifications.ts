...
export async function notifyMissionPrReady(
  ...
  opts: {
    ...
    prNumber: number;
    headSha: string;
    reason: 'pr_open' | 'auto_merge_blocked' | 'awaiting_review' | 'ci_failed' | 'base_retargeted';
    message: string;
  },
  ...
  notify({
    ...
    priority: (opts.reason === 'auto_merge_blocked' || opts.reason === 'ci_failed' || opts.reason === 'base_retargeted') ? 0 : -1,
  });
