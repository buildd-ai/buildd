...
if (!process.env.BUILDD_TEST_SERVER) {
  console.log(
    ...
    '   Example: BUILDD_TEST_SERVER=http://localhost:3000 bun test apps/runner/__tests__/integration-session-resume.test.ts',
  );
  process.exit(0);
}
...
beforeAll(async () => {
  ...
  try {
    ...
    if (config.builddServer !== testServer) {
      console.log(`Repointing runner: ${config.builddServer} → ${testServer}`);
