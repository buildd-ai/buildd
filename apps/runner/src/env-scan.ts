...
// Re-export pure parsing utilities from mcp-json.ts
export { extractVarReferences, parseMcpJsonContent } from './mcp-json';
export type { McpServerInfo } from './mcp-json';
import { parseMcpJsonContent as _parseMcpJsonContent } from './mcp-json';
import type { McpServerInfo } from './mcp-json';
...
export function parseMcpJson(filePath: string): McpServerInfo[] {
  try {
    const content = readFileSync(filePath, 'utf-8');
    return _parseMcpJsonContent(content);
  } catch {
    return [];
  }
}
