...
│ ○ Shipped     Waiting for next release › │  MissionReleaseSection → release
...
- **Task list:** md+ renders the same `MissionFeedList` as mobile — NEEDS YOU, MOVING NOW, then the phases, with the same folding rules — in the same max-width column. A row tap opens the same `?task=` sheet, docked on the right (pushState; Back closes it).
- **Timeline and Structure:** secondary views of the same tasks, folded into one md+ `Timeline · Structure` disclosure below the feed (closed by default). Inside it `MissionTabs` is the two-way toggle (`?view=structure`, `replaceState`).
...
- Footer rows: Orchestrator (a disclosure of the bookkeeping runs, at every width), `Records · N` (`MissionRecordsSheet`: `selectMissionRecords`, then "All artifacts" in the same sheet; `?artifact=` opens it), Notes (`MissionNotesSheet` in `MissionFeed.tsx`, which mounts the feed only when opened) and Settings. The page-bottom artifact dump is gone.
- Settings is a footer row like the others (44px). It also holds what used to crowd the header: renaming, the workspace link, next run, heartbeat badge, policy chip and initiative selector.
- **Description: one place, under the masthead (F5).** `MissionDescription` renders the mission description between the masthead and the situation, through the shared `MarkdownContent` renderer (`variant="compact"`, so headings stay body-sized), collapsed to about four lines behind "Show more" when long, with Edit in place (both 44px on mobile). Settings no longer carries the description and does not re-render the title: it offers "Rename mission", an input that opens prefilled. Mission cards still print no description (D7).
...
- **md and up.** `MissionFeedList` is the task list at every width. `CondensedTimeline` (`TimelineView`) and `StructureView` sit below it in a closed `Timeline · Structure` disclosure, behind the `MissionTabs` toggle (`?view=structure`, written with `replaceState`; a URL naming it opens the disclosure); they still group with the old chain classifier. The md+ time-axis strip renders inline under the masthead (`MissionFlightStripInline`) in place of the header pulse, which is `md:hidden`; a bar opens its task in the docked sheet. On mobile ⤢ opens `FlightDetailSheet` (D4).
- **Shipped (F6).** The Delivery Shipped step renders as one line for this mission — `Released` / `Partly released` / `Waiting for next release` — linking to the release that carries this mission's work (`MissionReleaseSection`; `loadMissionCarryingReleaseId` resolves it through `release_tasks` attribution, never the workspace's latest release). With no attributed release, or while the mission waits for the next one, it links to the workspace's releases list. The workspace queue ("N unshipped") and `Release now` are not on the mission page.
...
## Addendum: mission state model after the desktop review

- **F1. Waiting on you is active.** `missionNeedsYou(view)` (`lib/mission-state-view.ts`) is the one answer to "is the next step the owner's?". It is true when the state is `awaiting_merge`, `awaiting_verification`, `awaiting_decision` or `failing`, or when an unmerged PR, a failed deliverable or an owner decision is outstanding behind another verdict. A held mission's own ask (arm it) does not count. A criterion fact counts only as the verdict: while work is still moving, a failing "no open tasks" means the work is not finished, not that anyone is asking. `summarizeMissionForCard` derives the card's state once and stores it on the summary. `missionCardGroup` reads `needsYou` before the start gate, the pause and the schedule. A live worker gives `running`, a merge ask gives `review`, and anything else gives `attention`. The list header ("N active"), the list groups and Home all read `summary.group`, so they agree. A mission sits under PAUSED / HELD only when it is paused or held and nothing is waiting on the user.
- **F2. A failing structural criterion names its blockers.** For `no_open_tasks` the `criterion_failing` descriptor carries `blockers`: the open deliverables from the loaded rows, never from the stored evidence. The `[surface audit]` check is left out, which is the evaluator's own rule (`isNoOpenTasksCandidate` in `packages/core/mission-helpers.ts`). For `all_prs_merged` the blockers are the unmerged task PRs. When every failing criterion is `no_open_tasks`, the rows were loaded and none is open, the descriptor is `stale`. The headline then asks for re-verification instead of calling the criterion failing, because the verdict is older than the task state and the completion gate still reads it. The evaluator already excluded bookkeeping (orchestrator ticks, criteria evaluations, planning passes) and attempts through `isDeliverableTask`, and tests now pin that. Its stored evidence names each open task by title and status. The situation block prints one explanatory line under the headline (`situationDetail`). That line is the blockers (three, then "+N more", each opening the task sheet via `missionTaskHref`) or, for a criterion focus, the next action. Only other kinds get the causal claim with its ref. The criterion is no longer said three times.
- **F3. One count definition.** Every `n/N` reads `pulseDoneCounts` over `foldMissionDeliverables` rows: the detail header, the pulse, the card caption, the phase headers, `summarizeMissionForCard` (and so the card's grouping) and Delivery's Integrated step. The rows are work tasks, with attempts and cancelled or failed re-creations folded under their survivor and bookkeeping left out. **N** is every row except a cancelled one. **n** is the rows in `done`: completed with no PR, a merged PR or a closed PR. A completed task with an open PR is not done, because the merge is yours. A cancelled row still renders, with its own glyph `⊘` and a hollow segment, never queued grey. A folded phase segment carries `done` and `countable`, and its fill is `done / countable`. `missionDeliverableCounts(tasks)` gives the same numbers straight from tasks. The completion gate and the API still use `computeMissionProgress`, which is a separate, merge-strict definition that the gate owns.
- **F7a. No `0/0`.** The caption is empty when no row counts, or just `N live` when agents are running.
