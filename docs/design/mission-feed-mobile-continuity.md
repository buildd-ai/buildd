...
- **md and up.** `MissionFeedList` is the task list at every width. `CondensedTimeline` (`TimelineView`) and `StructureView` sit below it in a closed `Timeline · Structure` disclosure, behind the `MissionTabs` toggle (`?view=structure`, written with `replaceState`); they still group with the old chain classifier. The md+ time-axis strip renders inline under the masthead (`MissionFlightStripInline`) in place of the header pulse, which is `md:hidden`; a bar opens its task in the docked sheet. On mobile ⤢ opens `FlightDetailSheet` (D4).
- **Shipped (F6).** The Delivery Shipped step renders as one line for this mission — `Released` / `Partly released` / `Waiting for next release` — linking to the release (`MissionReleaseSection`). The workspace queue ("N unshipped") and `Release now` are not on the mission page.
