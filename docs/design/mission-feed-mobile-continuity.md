...
**Status:** Partially — S1 (pure model and vocabulary) is built; S2–S7 are proposed.
...
### Also found (fixed in slice S1)

At `page.tsx:590-603` the comment said `allTasks` is newest-first, but it is sorted ascending (`:576-578`). As a result `reviewerRetryMap` kept the **oldest** retry, not the newest. S1 replaced the loop with `buildReviewerRetryMap` (`lib/mission-helpers.ts`), which keeps the newest retry per parent regardless of input order (AC-21).
...
- D2: the `stalled` chip reads `STALLED`. `HEALTH_CHIP_CLASS` gives the Home/list health chip the same tokens as the detail chip, so BLOCKED is error-toned on both. The steering-cost stats (`MissionAuthorshipStats`) moved off the list cards and the detail header into the mission Settings panel under "Diagnostics".
- D3: `selectMissionCompletionSummary` picks the completion summary. In order it tries the completion-evaluation task, then an orchestrator summary written after the last deliverable finished, then the system "Mission completed" note. It never uses a work task or an attempt.
- The `reviewerRetryMap` fix (AC-21) landed here instead of in S3.
