...
- **Deviation (W1):** a bar tap in `FlightDetailSheet` **selects** the bar (`aria-pressed`, ring) instead of navigating, and "Open mission →" becomes "Open task in mission →" with `?task=<bar>` (AC-20). This is the same focus-first rule as the detail pulse: bars can be 2px wide, and a mis-tap should cost nothing.
