...
|---|---|---|---|---|---|
| 1 | Task category: `classifyTask`, `apps/web/src/lib/task-category.ts:24`, called from `POST /api/tasks` | Ordered keyword regex; returns null when nothing matches | `bug feature refactor chore docs test infra design review` | Low. Mostly a display tag. `review` changes claim pacing and reviewer handling, and the regex never emits it today. | **Best fit. Shadowed now (Point 9).** |
...
- **The question.** `TASK_CATEGORY_QUESTIONS` covers all nine stored categories, including `review`, which the regex cannot produce. It has contrastive definitions, no catch-all, and an instruction to follow definitions over title keywords. The state is `{ task: { title, description } }`, with the description truncated to 1,500 characters.
