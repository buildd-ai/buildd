...
1. **Get the value to the worker as a role env var.** Declare `ENV_NAME → label` on the role (`requiredEnvVars`, still read for this even though it's deprecated for MCP purposes) and/or as a workspace-wide default (`gitConfig.envMapping`; a role's own mapping wins on a shared key). At claim time (`role-env-injection.ts`) the label is resolved against the `secrets` table under a dedicated `role_env_secret` purpose (team/account/workspace scoping — see docs/credentials-architecture.md; deliberately not `mcp_credential`, which is kept out of the agent env by design) and the value rides the claim response as `roleEnvSecrets`. The runner's `resolveWorkerRoleEnv` merges it with whatever the local `env-mapping.json` resolution provides (today always empty — see below) before both the worktree install and the agent's `cleanEnv` see it; a declared label with no matching secret lands in `roleEnvMissing`/`missing` and surfaces as the same "Role env degraded" milestone. Store the value with `POST /api/secrets` (`purpose: "role_env_secret"`, `label` = the name used on the right of the mapping). Do not add registry tokens to the runner's `RUNNER_ENV_PASSTHROUGH` allowlist — secrets belong in buildd, scoped to the team/workspace that needs them, not in the host container.
   - The R2 role bundle's own `envMapping` stays packaged empty (`env-mapping.json` on disk resolves to nothing) — `requiredEnvVars` is deprecated in favour of `connectorRefs` for the MCP-config path, and this channel deliberately bypasses the bundle rather than reviving it there.
