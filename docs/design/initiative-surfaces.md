...
- **Arc-aware headline priority when multiple fire.** Resolved for the design: **waiting-on-you > milestone crossed > overnight count** (action beats celebration beats throughput). Left here only because the milestone-crossing detection (a real threshold delta vs. last visit, not just current %) is unspecified — a headline that says "crossed 80%" must not name an initiative whose card reads 68%.
...
3. **`InitiativeCard` + `InitiativeRail`** shared components + **`initiatives/page.tsx`** list + nav entry. Pure read over `GET /api/initiatives`. Ship the two contract stubs here so downstream phases inherit them:
   - Add **`hasLinearLink: boolean`** to the list route (one batched `externalLinks` existence query over all child mission ids); render the `C/LinearBadge` off it.
   - Sort **blocked-first, then `lastMotionAt = max(child.updatedAt)` desc**, derived client-side from the light payload. Zero-mission initiatives sort last. Status chip strictly `rollup.status`.
4. **Home rail** — mount the `InitiativeRail` above the Missions section; empty-collapse; leave un-filed + scheduled missions in place (reuse the phase-3 sort/badge derivation). Regression test mirrors `home-missions.test.ts` asserting orphan + scheduled missions still render.
5. **Arc-aware headline** — extend the subheading (`home/page.tsx:855`) with the ladder (waiting > milestone > count); milestone detection is a threshold-crossing delta, not current %. Unit-test in `greeting.test.ts`.
6. **Chips** on activity feed + waiting-on-you queue (scoping only).
