...
**Status:** Partially implemented — shipped in PR #2138 (merged 2026-09-06).
`tool-infra-error-v1` and the assembly record are built and emitting logs; the
assembly table and every plan-time recipe are not. Next phases:
...
`buildKnowledgeContext` (`knowledge-context.ts:94`) takes a single prose string
and fans it to five namespaces — `memory`, `plan`, `task`, `pr`, `code` — at
`topK: 3` each (`:110-122`). The same function, same signature, serves the
organizer planning a mission (`mission-context.ts:891`, query = title +
description) and a worker claiming a task (`context-injection.ts:333`, query =
...
`spec_compare` (`mcp-tools.ts:4864`) queries `docs` semantically, extracts
implementation anchors, then issues a **second lexical** query against `code`
using those anchors. The extractor is private (`mcp-tools.ts:5220`), reachable
...
| Exec | `tasks.subjectKind` (`schema.ts:1105`, indexed `:1169`) | `pull_request`, `error`, `mission`, `branch` | shipped; ignored by retrieval |
...
(`knowledge-store/pg-vector-store.ts:421` → `schema.ts:2302-2303`) but it is a global per-chunk
...
from active tasks' `pathManifest` (`schema.ts:1088`) and passes them as
...
`{ pattern, excerpt }` (`error-trace-scanner.ts:60`) — one raw line, no file, no
symbol, no exit code. There are no stack symbols to key on, which is why
`stack_symbol_query_hit` is reserved.

**Two extractors, not one.** `packages/core/error-signature.ts` (re-exported from
`apps/web/src/lib/error-signature.ts` for client-bundle callers) deliberately
...
`packages/core/error-signature.ts` and the strict slug validator in
...
hypothetical: `mcp-tools.ts:4872` records the same shape.
...
`missions.goalCriteriaState` `schema.ts:860`, plus `criteriaRearmFingerprint` /
...
**hit tracking** (`knowledge-store/pg-vector-store.ts:421`, global per-chunk counters, untouched)
...
queries the graph, and `BY_DESIGN_SKIP_REASONS` (`apps/web/src/lib/cbm-insight.ts:61`) records
...
`apps/runner/src/knowledge-ingest.ts:182` double-writes every docs file into `spec` as well,
...
8. **Then plan-time.** `conflict-v1`, querying `docs` in the shared path, and
   lifting `extractImplementationAnchors` out of `mcp-tools.ts:5220` into a
