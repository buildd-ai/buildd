...
`spec_compare` (`mcp-tools.ts:4303`) queries `docs` semantically, extracts
implementation anchors, then issues a **second lexical** query against `code`
using those anchors. The extractor is private (`mcp-tools.ts:4619`), reachable
...
| Exec | `tasks.subjectKind` (`schema.ts:971`, indexed `:1026`) | `pull_request`, `error`, `mission`, `branch` | shipped; ignored by retrieval |
...
(`pg-vector-store.ts:413` → `schema.ts:1811-1812`) but it is a global per-chunk
...
from active tasks' `pathManifest` (`schema.ts:954`) and passes them as
...
hypothetical: `mcp-tools.ts:4312` records the same shape.
...
`missions.goalCriteriaState` `schema.ts:783`, plus `criteriaRearmFingerprint` /
...
8. **Then plan-time.** `conflict-v1`, querying `docs` in the shared path, and
   lifting `extractImplementationAnchors` out of `mcp-tools.ts:4619` into a
