...
## ⚠️ URGENT: Active Items

### ~~August 5, 2026: Claude Opus 4.1 API Retirement~~ — DONE

### ~~August 17, 2026: Legacy Workbench + Experimental Prompt APIs Retired~~ — DONE

### ~~August 31, 2026: Sonnet 5 Pricing Sunset~~ — CANCELLED (GOOD NEWS)
...
## SDK Releases (September 1 – September 7, 2026)

7 CLI releases, 6 TypeScript SDK releases, 4 Python SDK releases this week.

### TypeScript SDK v0.3.257 – v0.3.263

| Version | Key Changes |
|---------|-------------|
| **v0.3.257** | `thinkingTokens` tracking; `resourceLinks` for MCP tool results; MCP server connection fixes; parity with CLI v2.1.257 (Fable 5.1 support) |
| **v0.3.258** | Parity with CLI v2.1.258 (macOS 12 launch hotfix, remote/scheduled session content fix) |
| **v0.3.259** | `user_message_uuids` tracking; `permissionPrompts: 'none'` option for unattended headless hosts |
| **v0.3.260** | `user_message_uuid` on thinking tokens; latency breakdowns; `rewindFiles` fixes |
| **v0.3.261** | Plugin delivery via stdin; fixed `Symbol.dispose` issues on Node ≤22 and under Jest/Vitest |
| **v0.3.263** | Parity with CLI v2.1.263 |

### Python SDK v0.2.149 – v0.2.152

All four releases are CLI bundle bumps (v2.1.252 → v2.1.259). No functional changes beyond the bundled CLI version.

### CLI v2.1.257 – v2.1.263

| Version | Date | Key Changes |
|---------|------|--------------|
| **v2.1.257** | Sep 1 | **Claude Fable 5.1** (`claude-fable-5-1`) support — 1M context, $10/$50/MTok, cache reads now $0.25/MTok (was $1.00); "Time format"/`timeZone` settings; **Containment Escape** rule added to auto mode; `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` env var; `/effort` session-only mode; `/doctor` warning for stale sandbox mask files; one-time prompt for file reads outside the working directory |
| **v2.1.258** | Sep 1 | Hotfix: Claude Code failing to launch on macOS 12 Monterey (regression from v2.1.255); fixed remote/scheduled sessions failing with "user messages must have non-empty content" after re-sent permission approvals |
| **v2.1.259** | Sep 2 | `managedMcpServers` managed setting for org-enforced HTTP/SSE MCP servers; `--permission-prompts none` for unattended headless hosts; GitLab MR recognition improvements; `--json` on `claude plugin validate` |
| **v2.1.260** | Sep 3 | Diff panel for fullscreen mode (`/diff`); prompt-cache miss causes surfaced in `/cost` and statusline; `/reload-plugins` now works in headless sessions; text form of `/advisor` for desktop + headless |
| **v2.1.261** | Sep 4 | "Organization policy" diagnostic line in `/status`/`claude doctor`; `bashOutputMaxChars`/`taskOutputMaxChars` settings (raise inline command/task output up to 128K chars before spilling to a file); `--append-subagent-system-prompt-file`; `/skill-doctor` (lists unused skills + their context cost); large batch of sandbox/session/VS Code fixes |
| **v2.1.263** | Sep 6 | Bug fixes and reliability improvements |

---

## New Platform Features (September 1–7, 2026)

### Claude Fable 5.1 + Mythos 5.1 (September 1)

Anthropic's "triple release" day. Fable 5.1 is the GA mass-market model; Mythos 5.1 is the same underlying model gated to vetted cybersecurity/life-sciences orgs. Headline pricing unchanged ($10/$50 per MTok) but **cache-read price cut 75%** ($1.00 → $0.25/MTok). Fable 5.1 matches or beats Fable 5 at low/medium effort and is notably stronger at high effort tiers. Available on Claude API, AWS, GCP, and Azure at GA.

**Relevance for Buildd**: The cache-read cut is the actionable item — see recommendation #1. Buildd should also add `claude-fable-5-1` to role model options alongside `claude-fable-5`.

### Enterprise Frontier Safeguards (EFS) — new, free enterprise compliance tier

Announced alongside Fable 5.1: zero data retention, customer-controlled storage (AWS S3, Azure Blob, or GCS) for enterprise/regulated customers, with no Anthropic human review path. Co-developed with 100+ institutions (including major financial firms). Offered at no additional cost — explicitly positioned as a switching-cost moat for regulated customers ahead of Anthropic's IPO.

**Relevance for Buildd**: Directly adjacent to Buildd's existing "no prod DB in worker sandboxes" security boundary (see CLAUDE.md → Worker Sandbox Constraints). Worth a co-messaging angle for enterprise workspaces: "Buildd workers never touch production credentials, and can now run on Anthropic infrastructure with zero data retention and customer-controlled storage." No engineering action — a docs/positioning opportunity.

### `/skill-doctor` — unused-skill + context-cost audit (CLI v2.1.261)

New command lists skills that are loaded but never invoked in a session, along with their context-token cost. Directly analogous to what Buildd's own role/skill system needs at the workspace level — Buildd currently has no equivalent to flag a `workspaceSkills` row that's configured but never selected by any role/task.

**Relevance for Buildd**: Build an analogous audit — e.g., a `GET /api/workspaces/[id]/skills/unused` check or a dashboard callout — surfacing skills with `isRole: false` (or roles) that haven't been referenced by any task in N days, with their packaged CLAUDE.md/context size from `role-config.ts`. Effort: Medium.

### `managedMcpServers` — org-enforced MCP config (CLI v2.1.259)

New managed setting lets an org admin push a fixed set of HTTP/SSE MCP servers that individual sessions cannot override or remove — closes a gap where org policy could previously only *restrict* tools, not *require* specific MCP servers.

**Relevance for Buildd**: Enterprise workspaces could use the same pattern for role `mcpServers` — currently role-level `mcpServers` config is opt-in per role; an admin-enforced floor (e.g., "every role in this workspace must mount the compliance-logging MCP server") isn't expressible today. Low priority unless an enterprise customer asks. Effort: Medium.

### `bashOutputMaxChars` / `taskOutputMaxChars` (up to 128K, CLI v2.1.261)

Raises how much command/background-task output is inlined before being spilled to a file. Buildd workers running large build/test suites currently may hit truncation earlier than the CLI now supports natively.

**Relevance for Buildd**: Verify `packages/core/worker-runner.ts` doesn't hardcode a lower output-capture ceiling than the CLI now allows — a mismatch would silently truncate worker tool output before the CLI's own limit kicks in. Effort: Low (verification).

### `--permission-prompts none` (CLI v2.1.259, SDK `permissionPrompts: 'none'` v0.3.259)

Purpose-built for unattended headless hosts — suppresses permission-prompt stalls entirely rather than requiring `bypassPermissions` or pre-approved rules. This is closer to Buildd workers' actual operating mode (headless, unattended, sandboxed) than the options previously available.

**Relevance for Buildd**: Evaluate replacing whatever permission-mode flag Buildd's worker launch currently uses in `packages/core/worker-runner.ts` — this may be a more precise fit than broad `bypassPermissions`, since it targets exactly "don't block on a prompt" without disabling permission *evaluation*. Effort: Low (investigation + swap).

### `ant apply` — agents/skills/environments as code (`ant` CLI, ongoing; v1.30.0 this week)

Not new this week (shipped alongside Managed Agents in April), but the `ant` CLI hit v1.30.0 during this scan window and continues to add resource types. `ant apply` declares agents, skills, environments, memory stores, and scheduled deployments as Markdown/YAML/JSON files in a repo, reconciled against the Claude API via a plan/apply flow with a committed `claude-lock.json` for drift detection.

**Relevance for Buildd**: Architecturally close to what Buildd's `role-config.ts` already does (bundle CLAUDE.md + `.mcp.json` + env → package → deploy), just for Anthropic's own Managed Agents product rather than Buildd's workers. Worth watching as a convergence signal — if Anthropic's Managed Agents primitives (session budget, advisor model, geo-pinning, and now IaC-style deployment) mature further, evaluate whether Buildd should treat Managed Agents as a worker backend option rather than only self-hosted Claude Code. No action now — track.

---

## Anthropic Business News (September 1–7, 2026)

### IPO: Public S-1 Expected This Week

Reporting indicates Anthropic plans to make its confidentially-filed S-1 prospectus public shortly after Labor Day (Sept 7, 2026), targeting an October 2026 Nasdaq listing at a reported ~$2 trillion target valuation — roughly double the ~$965B private valuation from the May 2026 Series H-1 round. The Sept 1 Fable 5.1/EFS/pricing announcements are widely read as pre-IPO positioning (aggressive agentic pricing + a gated capability tier + a privacy-first enterprise tier, all in one day).

**Relevance for Buildd**: No direct action. Worth tracking for potential pricing/API stability changes around the actual listing.

---

## New Ecosystem Projects (Since August 31, 2026)

| Project | Stars | Description |
|---------|-------|--------------|
| **obra/superpowers** | ~265K (up from ~94K on Aug 31) | Continued explosive growth for the TDD-enforcing skills framework — now one of the highest-velocity repos in the ecosystem, not just the skills category. |
| **NVIDIA/skills** | New | Official NVIDIA Agent Skills pack — installs into Claude Code, Codex, and other coding agents to run Physical AI, robotics, simulation, CUDA, and RAG workflows end-to-end. Signals large vendors now shipping first-party Claude Code skill packs, not just prompt/framework repos. |
| **linny006/trending-claude-skills** | New (tool) | Auto-updated leaderboard scraping GitHub Search API for recently-created/updated agent-skill repos, refreshed every 15 minutes — ranks by freshness/momentum rather than all-time stars. Useful as a lower-latency source for next week's scan. |

**Ecosystem trend this week**: Skills-layer growth (Superpowers, NVIDIA) continues to outpace raw framework/orchestration repos, consistent with the Aug 31 note. First-party vendor skill packs (NVIDIA) are a new sub-trend worth watching — if hardware/platform vendors standardize on shipping Claude Code skills as a distribution channel, Buildd's skill-import path (marketplace/plugin install) becomes more valuable as a differentiator.

---

## Recommendations for Buildd

### This Week (September 7, 2026)

**#0 — Bump SDK to ^0.3.263 / Python to ^0.2.152 / CLI to v2.1.263**
No breaking changes this week. Location: `packages/core/package.json`. Effort: Trivial.

**#1 — ⚠️ Reflect the 75% cache-read price cut in budget forecasts and role cost estimates**
Cache reads dropped from $1.00 to $0.25/MTok (Sept 1). Buildd's context-heavy worker sessions (long-running tasks with repeated prompt-cache hits) are exactly the shape of workload this benefits most — reporting suggests up to ~45% savings for agentic workloads. Update `apps/web/src/lib/budget-forecast.ts` (or equivalent) rate table and any role-card cost estimates that hardcode the old cache-read rate. This is a customer-facing win worth surfacing (lower effective Buildd task cost), not just an internal number update. Effort: Low. Priority: High (visible cost regression in forecasts if left stale).

**#2 — Add `claude-fable-5-1` to role model options**
GA replacement/companion to `claude-fable-5` with better high-effort performance at the same headline price. Location: `apps/web/src/lib/role-config.ts`. Effort: Low.

**#3 — Verify worker output-capture limits against `bashOutputMaxChars`/`taskOutputMaxChars` (128K)**
Check `packages/core/worker-runner.ts` for a hardcoded output-capture ceiling lower than the CLI's new 128K inline limit — a mismatch would truncate tool output before the CLI itself would. Effort: Low (verification).

**#4 — Evaluate `--permission-prompts none` for worker launch (replaces broad `bypassPermissions`?)**
Purpose-built for unattended headless hosts like Buildd workers — suppresses prompt stalls without disabling permission evaluation entirely, unlike `bypassPermissions`. Location: `packages/core/worker-runner.ts`. Effort: Low (investigation + swap).

**#5 — Build a `/skill-doctor`-equivalent audit for workspace skills**
New CLI command surfaces unused skills + their context-token cost per session. Buildd has no workspace-level equivalent for `workspaceSkills` rows that are configured but never selected by any role/task. Location: new endpoint under `apps/web/src/app/api/workspaces/[id]/skills/`, using `role-config.ts` packaging size data. Effort: Medium.

**#6 — Track Enterprise Frontier Safeguards (EFS) as a co-messaging opportunity**
Zero data retention + customer-controlled storage (S3/Blob/GCS), free, targeted at regulated enterprise customers. Pairs naturally with Buildd's existing "no prod DB in worker sandboxes" boundary. Docs/positioning only — no engineering action. Effort: Low (copy).

**#7 — Investigate `managedMcpServers` for org-enforced role MCP config**
Lets an org admin require (not just restrict) specific MCP servers across all sessions. Buildd's role-level `mcpServers` is currently opt-in only; no admin floor exists. Low priority absent a specific enterprise ask. Effort: Medium.

**#8 — Track Managed Agents / `ant apply` maturity as a potential alternate worker backend**
Not actionable yet, but Managed Agents (session budget, advisor model, geo-pinning, IaC-style deploy via `ant apply`) is converging on primitives Buildd already has for self-hosted workers. Revisit if Managed Agents adds anything Buildd workers structurally cannot do (e.g., true zero-infra hosting for customers who don't want any worker infra at all).

### Still Relevant (From August 31, 2026)

**#9 — Wire `PreModelSwitch`/`PostModelSwitch` hooks for a model-switch audit trail** (SDK v0.3.251)
**#10 — Use `--restricted` flag for reviewer/read-only roles** (CLI v2.1.248)
**#11 — Use `modelUsage[*].costBasis` for Managed Agents billing accuracy** (SDK v0.3.246)
**#12 — Hide `ambient` tasks from mission task views** (SDK v0.3.247)
**#13 — Use `experimental.cacheTtl` for long-running batch workers** (CLI v2.1.248)
**#14 — Track Claudeforce for enterprise CRM integration opportunity**
**#15 — Use `perTaskStopAffordance` for graceful interrupt on long tasks** (SDK v0.3.246)
**#16 — Enable cross-session messaging on Bedrock/Vertex/Foundry workers** (CLI v2.1.248)

### Still Relevant (From August 17–24, 2026)

**#17 — ⚠️ BREAKING: Re-add task tools to worker `allowedTools` after SDK v0.3.233 bump** — still unresolved, check before any SDK bump in #0 above
**#18 — Use `PostToolUse` `classifierContext` for auto-mode decisions** (SDK v0.3.236)
**#19 — Use `SDKContextUsage` from `/context` for context monitoring** (SDK v0.3.232)
**#20 — Wire cross-session @mentions and `/hold`/`/refuse` for peer worker messaging** (CLI v2.1.232)
**#21 — Pass GitLab MR URLs directly to `--worktree`** (CLI v2.1.232/v2.1.233; GitLab support keeps expanding — v2.1.259 added more recognition improvements)
**#22 — Audit MCP server compatibility with MCP 2026-07-28 spec**

### Still Relevant (Older)

**#23 — Fix Python SDK skill-name injection** (v0.2.129 security fix) — still critical if unresolved
**#24 — Investigate `claude self-hosted-runner` as a Buildd Enterprise deployment mode**
**#25 — Expose `sandbox.filesystem.disabled` in role configuration**
**#26 — Surface `api_error_status: 529` in task error UI**
**#27 — Evaluate `codebase-memory-mcp` for token reduction** (32K-star project)
**#28 — Use `agentProgressSummaries` for live task visibility** (v0.3.162+)
**#29 — OpenTelemetry worker observability**
**#30 — `SessionStore` for transcript persistence** (alpha)

---

## SDK Releases (August 24 – August 31, 2026)

7 CLI releases, 6 TypeScript SDK releases, 4 Python SDK releases this week.

### TypeScript SDK v0.3.245 – v0.3.251

| Version | Key Changes |
|---------|-------------|
| **v0.3.245** | Parity with CLI v2.1.245 (glibc crash fix) |
| **v0.3.246** | `user_message_uuid` links replies to triggering messages; `modelUsage[*].costBasis` field (`'list' \| 'managed' \| 'unknown'`) — distinguishes API-key vs Managed Agents billing; `modelPricing` in `managedSettings` for host-provided rate table; `perTaskStopAffordance` option for controlling interrupt stop behavior |
| **v0.3.247** | `ambient` boolean flag on task entries (`task_started`, `task_notification`, `background_tasks_changed`) — marks housekeeping/internal tasks that hosts should exclude from user-facing views; Fixed `permissionMode` reporting stale values after mode switches |
| **v0.3.248** | Per-server `timeout` on SDK-hosted MCP servers via `createSdkMcpServer({ timeout })` — overrides `MCP_TOOL_TIMEOUT` globally |
| **v0.3.250** | Parity with CLI v2.1.250 (bug fixes) |
| **v0.3.251** | `PreModelSwitch` and `PostModelSwitch` hook events — fire before/after any model switch in a session; Live streaming of foreground subagent tool calls and results to Remote Control clients; **Security**: fixed symlink vulnerability in file tools; **Security**: fixed marketplace plugin path-traversal vulnerability |

### Python SDK v0.2.145 – v0.2.148

All four releases are CLI bundle bumps (v2.1.247 → v2.1.251). No functional changes beyond the bundled CLI version.

### CLI v2.1.245 – v2.1.251

| Version | Date | Key Changes |
|---------|------|-------------|
| **v2.1.245** | Aug 25 | Fixed crash on startup on Linux distributions with glibc 2.44 |
| **v2.1.246** | Aug 25 | Auto mode tab in `/permissions` dialog; Turn completion time now displayed; Transcript rendering performance fix for long lines; Background session stability improvements |
| **v2.1.247** | Aug 26 | `SendFeedback` tool for drafting in-session feedback reports; Custom tip rotation via `spinnerTipsOverride`; `/claude-api cost-optimize` subcommand for spend profiling; Admin API coverage added to `/claude-api` skill |
| **v2.1.248** | Aug 27 | **New**: `--restricted` flag — removes all command/code execution tools and enforces strict file-access limits (read-only by default); **New**: `experimental.cacheTtl` setting for per-agent prompt cache TTL override; **New**: `claude self-hosted-runner --client-label` for labeling runner instances; Cross-session messaging now works on Bedrock, Vertex, and Foundry (was local-only); Multiple prompt-cache and security fixes |
| **v2.1.250** | Aug 28 | Bug fixes and reliability |
| **v2.1.251** | Aug 28 | `PreModelSwitch`/`PostModelSwitch` lifecycle hooks; Live streaming of foreground subagent tool calls/results to Remote Control clients; Spend limit bar in `/usage` command; Per-session prompt-cache line in `/cost` (hit ratio, misses, tokens re-cached, warm/cold); **Security**: fixed symlink vulnerability in file tools; **Security**: fixed marketplace plugin path-traversal vulnerability |

---

## New Platform Features (August 24–31, 2026)

### Salesforce × Anthropic: Claudeforce (August 26)

Salesforce and Anthropic announced **Claudeforce** — Claude becomes the default reasoning engine across the entire Salesforce ecosystem (Agentforce, Slack, and more). The launch includes:

- **"Salesforce in Claude" plugin**: 37 prebuilt sales skills (meeting prep, deal health, pipeline review). Pilot customers active; open beta expected September 2026.
- **Claude in Agentforce**: Claude's reasoning embedded in Salesforce's agentic platform, with Salesforce data, workflows, business logic, and governance accessible.
- **Benioff context**: Salesforce CEO framed this as the response to the "SaaSpocalypse" concern — Claude + Salesforce governance as the enterprise AI defense against unmanaged agents.

**Relevance for Buildd**: Large enterprises using Salesforce are now encountering Claude natively. Buildd can position as the coordination layer for mixed Salesforce-Claude + Buildd-worker workflows — e.g., Salesforce opportunity triggers Buildd tasks, worker results post back to Salesforce. No immediate engineering action; watch for the September open beta for integration specifics.

### Claude for Teachers — Enterprise GA (August 28)

Claude for Teachers is now available to U.S. K-12 schools and districts as a **free Enterprise offering**, including premium capabilities (teaching skills grounded in learning science, academic standards integration for all 50 states). Qualifying orgs that sign up by June 30, 2027 get a full year of free access.

**Relevance for Buildd**: Low direct relevance unless Buildd plans an education vertical.

### Claude Team Plan for Scientists (August 28)

Anthropic is giving **10,000 scientists** access to Claude through a discounted Team plan: standard seats free, premium seats (5× usage limits) at $15/month (80% discount, locked for one year).

**Relevance for Buildd**: Signal that Anthropic is aggressively pricing to gain users in technical domains. The usage-limit promotions increase Claude Code platform traffic broadly.

### `--restricted` Mode for Headless Workers (CLI v2.1.248)

A new `--restricted` flag strips all command/code execution tools from a session and enforces read-only file access. Designed for review-only or analysis-only agent roles where code execution is not desired.

**Relevance for Buildd**: This is exactly what Buildd's "read-only" or "reviewer" role personas need. The `--restricted` flag can replace manual `allowedTools` exclusion lists for reviewer roles. Wire it into role config as a `restrictedMode: true` boolean that maps to `--restricted` in the worker launch args.

### Security: Two Vulnerabilities Fixed (CLI v2.1.251)

1. **Symlink vulnerability in file tools** — file tools could follow symlinks outside the working directory, allowing reads/writes to unintended paths.
2. **Marketplace plugin path-traversal** — plugin install from marketplace could write files outside the plugin directory.

Both are fixed in v2.1.251. **All Buildd workers should be upgraded immediately.**

---

## Anthropic Business News (August 24–31, 2026)

### IPO: S-1 Still Imminent

Public S-1 filing still expected August–September. Nasdaq listing targeted October 2026, prediction market median November 30. The Claudeforce deal with Salesforce ($billions enterprise contract) likely boosted the pre-IPO narrative.

---

## New Ecosystem Projects (Since August 24, 2026)

| Project | Stars | Description |
|---------|-------|-------------|
| **Karpathy Skills** (behavioral guardrails) | 207K | Andrej Karpathy posted agent failure patterns in Jan 2026; community turned them into a CLAUDE.md with 4 behavioral guardrail principles. Now one of the fastest-growing AI workflow repos ever. |
| **Superpowers** | 94K | TDD-enforcing skills framework — turns Claude Code into a senior dev that follows test-driven development. Official Anthropic marketplace. Most-starred Claude Code skills repo of 2026. |
| **Agent Skills Pack** (Addy Osmani) | 90.1K | 24 production-grade engineering skills covering the full development lifecycle. One of the most-installed skill packs of 2026. |
| **OmniRoute** | Growing | MIT-licensed local AI gateway unifying 290+ LLM providers and local models under a single OpenAI-compatible endpoint. Proprietary token compression + intelligent fallback across Claude Code, Cursor, Cline, and Copilot. |

**Ecosystem trend this week**: Community skills and workflow frameworks are now outpacing infrastructure tooling in star velocity. The skills layer (Superpowers, Karpathy guardrails, Osmani pack) is where the most user value is being created. Buildd's role/skill system is aligned with this trend — the `sdk-changelog-monitor` skill and `buildd-workflow` skill are Buildd's equivalents.

---

## Recommendations for Buildd

### This Week (August 31, 2026)

**#0 — ⚠️ SECURITY: Upgrade workers to CLI v2.1.251+ immediately**
Two security vulnerabilities fixed in v2.1.251: (1) file tools could follow symlinks outside the working directory; (2) marketplace plugin install could path-traverse outside the plugin directory. Both are patched in v2.1.251. Workers running v2.1.241–v2.1.250 are exposed. Update `packages/core/package.json` to `^0.3.251` (TS SDK) and `^0.2.148` (Python SDK) immediately. Effort: **Trivial. Priority: CRITICAL.**

**#1 — Bump SDK to ^0.3.251 / Python to ^0.2.148**
Covers the security fixes above plus all new features this week. Location: `packages/core/package.json`. Effort: Trivial.

**#2 — Wire `PreModelSwitch`/`PostModelSwitch` hooks for model audit trail (SDK v0.3.251)**
New hook events fire before and after any model switch in a worker session. Buildd can use these to: (a) log model switches to the task event timeline, (b) enforce role-level model restrictions (e.g., reject a switch to a model not in the role's allowed tier), and (c) record actual models used per turn for billing accuracy. Location: `apps/runner/src/workers.ts` (hook config). Effort: Low.

**#3 — Use `--restricted` flag for reviewer/read-only roles (CLI v2.1.248)**
The new `--restricted` flag removes all code/command execution tools and enforces read-only file access — ideal for Buildd's reviewer or analyst role personas. Add `restrictedMode: boolean` to role config, mapping to `--restricted` in worker launch args. Replaces manual `allowedTools` exclusion lists for these roles. Location: `apps/web/src/lib/role-config.ts`, `packages/core/worker-runner.ts`. Effort: Low.

**#4 — Use `modelUsage[*].costBasis` for Managed Agents billing accuracy (SDK v0.3.246)**
The new `costBasis` field (`'list' | 'managed' | 'unknown'`) on each `modelUsage` entry distinguishes API-key vs Managed Agents billing. Buildd's budget forecast card currently applies uniform rate assumptions — this field enables per-turn billing routing: `'managed'` sessions use seat-based pricing, `'list'` uses API rates. Location: `apps/web/src/lib/budget-forecast.ts` (or equivalent). Effort: Low.

**#5 — Hide `ambient` tasks from mission task views (SDK v0.3.247)**
The new `ambient: true` flag on task entries marks housekeeping/internal tasks that should be excluded from user-facing views. Buildd's mission task grid and task sidebar should filter out `ambient` tasks the same way a standard Claude Code host would. Prevents internal SDK bookkeeping from cluttering mission views. Location: `apps/web/src/app/app/(protected)/tasks/TaskGrid.tsx`. Effort: Low.

**#6 — Use `experimental.cacheTtl` for long-running batch workers (CLI v2.1.248)**
The new per-agent `experimental.cacheTtl` setting overrides the default prompt cache TTL. For Buildd's nightly batch workers or mission workers that run multi-hour sessions, tuning this TTL can reduce cache misses and lower API costs. Location: `apps/runner/src/workers.ts` (worker launch config). Effort: Low.

**#7 — Track Claudeforce for enterprise CRM integration opportunity**
Salesforce × Anthropic "Claudeforce" launched August 26 — Claude is now default reasoning in Agentforce + Slack. For enterprise Buildd workspaces that use Salesforce, this creates a workflow integration opportunity: Salesforce opportunity → Buildd task → worker result → Salesforce update. The "Salesforce in Claude" plugin open beta is expected September 2026 — evaluate integration then. File as a product backlog item. Effort: Investigation.

**#8 — Use `perTaskStopAffordance` for graceful interrupt on long tasks (SDK v0.3.246)**
New option controls interrupt stop behavior per task — useful for Buildd workers handling long mission tasks where a mid-task interrupt should complete the current unit of work before stopping. Location: `packages/core/worker-runner.ts`. Effort: Low.

**#9 — Enable cross-session messaging on Bedrock/Vertex/Foundry workers (CLI v2.1.248)**
Cross-session messaging (introduced locally in v2.1.224) now works on Bedrock, Vertex, and Foundry deployments. If any Buildd enterprise customers run workers on these cloud backends, peer-to-peer worker messaging is now available without routing through Buildd's API. Effort: Trivial (verify version).

### Still Relevant (From August 24, 2026)

**#10 — ⚠️ BREAKING: Re-add task tools to worker `allowedTools` after SDK v0.3.233 bump** — still unresolved
**#11 — Use `PostToolUse` `classifierContext` for auto-mode decisions** (SDK v0.3.236)
**#12 — Use `SDKContextUsage` from `/context` for context monitoring** (SDK v0.3.232)
**#13 — Surface `ANTHROPIC_DEFAULT_MODEL` env var in role config** (CLI v2.1.236)
**#14 — Audit MCP server compatibility with MCP 2026-07-28 spec**
**#15 — Audit prompt caching fix for gateway deployments** (v2.1.237)

### Still Relevant (From August 17, 2026)

**#16 — Use `terminal_slash_commands` to filter Remote Control command display** (TS SDK v0.3.227)
**#17 — Wire cross-session @mentions and /hold//refuse for peer worker messaging** (CLI v2.1.232)
**#18 — Pass GitLab MR URLs directly to `--worktree`** (CLI v2.1.232)
**#19 — Use `claude remote-control --continue` for session resume** (CLI v2.1.229)

### Still Relevant (From August 10, 2026)

**#20 — Fix Python SDK skill-name injection** (v0.2.129 security fix) — still critical
**#21 — Use `system/permission_denied` events for task timeline** (TS SDK v0.3.223)
**#22 — Add `resumeDropsTurn` to interrupt/resume flow** (TS SDK v0.3.223)
**#23 — Investigate `claude self-hosted-runner` as Buildd Enterprise deployment mode**
**#24 — Expose `sandbox.filesystem.disabled` in role configuration**
**#25 — Surface `api_error_status: 529` in task error UI**

### Still Relevant (Older)

**#26 — Use `tool_result_meta` for denied/interrupted tool classification** (SDK v0.3.216)
**#27 — Wire `cancel_queued` to interrupt endpoint** (SDK v0.3.219)
**#28 — Fix billing accuracy with `canonicalModel` + `provider`** (SDK v0.3.218)
**#29 — Surface `fast_mode_disabled_reason` in role/task UI** (SDK v0.3.219)
**#30 — Add `terminal_reason` to task completion payload** (Python v0.2.126)
**#31 — Expose subagent depth and concurrency caps in mission/role config** (SDK v0.3.217)
**#32 — Adopt `sandbox.network.strictAllowlist` for locked-down roles** (SDK v0.3.219)
**#33 — Evaluate `codebase-memory-mcp` for token reduction** (32K-star project)
**#34 — Adopt mid-conversation tool changes beta** (Fable 5/Opus 5)
**#35 — Use `agentProgressSummaries` for live task visibility** (v0.3.162+)
**#36 — OpenTelemetry worker observability**
**#37 — `SessionStore` for transcript persistence** (alpha)

---

## SDK Releases (August 17 – August 24, 2026)

9 CLI releases, 10 TypeScript SDK releases, 9 Python SDK releases this week.

### TypeScript SDK v0.3.232 – v0.3.241

| Version | Key Changes |
|---------|-------------|
| **v0.3.232** | `tool_result` frames with `_meta` now emit `{ content, _meta }` (matching main-loop frames); `/context` now carries structured `SDKContextUsage` payload (no more markdown table parsing); `vcs_state_changed` populates `branch` field for push operations |
| **v0.3.233** | **BREAKING**: Todo/task-tracking tools (`TaskCreate`, `TaskGet`, `TaskUpdate`, `TaskList`, `TodoWrite`) removed from default tool surface on Opus 4.8+, Sonnet 5, Fable 5, Mythos 5 — must explicitly name them in `tools` option or `allowedTools` (or set `CLAUDE_CODE_ENABLE_TODO_TOOLS=1`); Notification hooks now fire for pending permission prompts on SDK path |
| **v0.3.234** | `ApiKeySource` type updated to match actual values emitted by `system/init`; `vcs_state_changed` reports directory after inner `cd`; peer origin `fromMode` allows same-class message delivery without approval; `SDKSystemMessage` gains optional `effort` field |
| **v0.3.235** | Parity with CLI v2.1.235 (spellcheck, markdown fixes) |
| **v0.3.236** | `PostToolUse` hooks can return `hookSpecificOutput.classifierContext` — a host-asserted note the auto-mode permission classifier reads alongside the tool result |
| **v0.3.237** | Parity with CLI v2.1.237 (prompt-cache security fix, Concise output style) |
| **v0.3.238** | `is_backgrounded` and `spawn_depth` on `task_started` events; `suppressOriginalPrompt` in `UserPromptExpansion`; cross-session peer messages now report `refused` lifecycle state when receive-side policy declines; fixed hooks not applying after host re-sends `initialize` |
| **v0.3.239** | `total_cost_usd` / `modelUsage.costUSD` now include 1.1× US-only-inference multiplier when response reports `inference_geo: "us"`; fixed `SYSTEM_PROMPT_DYNAMIC_BOUNDARY` in array systemPrompt being sent as literal text on Bedrock/Vertex/Foundry/gateway; reconnecting hosts now receive a `background_tasks_changed` snapshot |
| **v0.3.240** | Parity with CLI v2.1.240 |
| **v0.3.241** | Parity with CLI v2.1.241 |

### Python SDK v0.2.136 – v0.2.144

Primarily CLI bundle bumps (v2.1.233 → v2.1.241). No functional changes beyond the bundled CLI version.

### CLI v2.1.233 – v2.1.241

| Version | Date | Key Changes |
|---------|------|-------------|
| **v2.1.233** | Aug 14 | GitLab MR URL support in `--worktree` and `claude agents` view; `CLAUDE_CODE_TOOL_MEMORY_LIMIT` for Bash memory cgroup on Linux; `CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS` for WebFetch cache config; `forward_user_identity` opt-in for apps gateway; **Security**: Windows `\??\` path bypass fix (NTLM credential leak); Notification hooks now fire for permission prompts |
| **v2.1.234** | Aug 17 | **Auto-continue** when claude.ai usage limit resets; `CLAUDE_CODE_PROJECT_DIR_NAME` env var for per-project transcript directories; GitLab MR badge in footer/statusline; `selection:clear` keybinding; **Security**: Remote file reads/restore reject Windows NT-namespace paths |
| **v2.1.235** | Aug 18 | Optional `spellcheck` setting using `aspell`, `hunspell`, or `ispell`; markdown layout fixes |
| **v2.1.236** | Aug 19 | `ANTHROPIC_DEFAULT_MODEL` env var to set session default model; `notify_when_idle` on cross-session `SendMessage` (macOS/Linux); sandbox macOS wildcard deny-rule improvements |
| **v2.1.237** | Aug 20 | **Security**: Fixed prompt caching for sessions using LLM gateway or custom base URL; Added built-in "Concise" output style (leads with result, skips preamble) |
| **v2.1.238** | Aug 20 | `keybindingFlavor: "readline"` for Bash-style word deletion; MCP plugin marketplace `headersHelper` for auth header minting; `claude self-hosted-runner --defer-shutdown-max-min` for graceful shutdown; `--proxy-authorization-command`/`--proxy-authorization-file`; **Memory leak fixed**: unbounded memory growth in long sessions (subagent tool results now released) |
| **v2.1.239** | Aug 21 | Cost estimates include 1.1× US-only-inference premium for data-residency workspaces; fullscreen renderer now available on Bedrock/Vertex/Foundry; `/claude-api upgrade` helper to migrate Python from anthropic 0.x to 1.x; Windows cross-session messaging via `SendMessage`/`ListAgents`; synced cloud plugins show as `name@synced`; 40+ bug fixes |
| **v2.1.240** | Aug 22 | Bug fixes and reliability |
| **v2.1.241** | Aug 23 | Bug fixes and reliability |

---

## New Platform Features (August 17–24, 2026)

### ⚠️ Sonnet 5 Pricing Permanently Frozen (August 10 announcement, action needed now)

Anthropic announced on August 10 that Sonnet 5's introductory pricing ($2/$10/MTok) is now **permanent**. The scheduled September 1 reversion to $3/$15 will not occur.

**Action for Buildd**: Remove any billing alerts or copy referencing the September 1 price increase. The new tokenizer (~30% more tokens for same text vs Sonnet 4.6) is still a real cost factor — consider surfacing this in the budget forecast card as a "tokenizer multiplier" note rather than a price alert.

### Claude Managed Agents: Session Budget, Advisor Model, Inference Geo Pinning

Three new governance features for Claude Managed Agents (GA this week):

**Session budget control**: Set a hard cap (`budget`) on a session's spend. Sessions that reach the limit pause with a `budget_reached` stop reason; raising or removing the budget resumes them. No polling needed — clean event-driven.

**Advisor model**: Configure a secondary model (`{"type": "advisor"}`) in the agent's multiagent roster that the primary thread can consult mid-turn for strategic guidance. The advisor must be at least as capable as the primary model.

**Inference geo pinning**: Control where model inference runs per-agent or per-session (`inference_geo: "us"` billed at 1.1×, `"global"` at standard rates).

**Relevance for Buildd**: The session budget feature is Anthropic's Managed Agents equivalent of Buildd's `costBudgetUsd` on missions. Buildd already has better mission-level cost governance; these features are parity signals confirming the pattern. The 1.1× US-inference premium now shows up in SDK `total_cost_usd` values — Buildd's budget forecast card needs to reflect this for workspaces in data-residency mode.

### Admin API User Management Now GA

Enterprise organization member management endpoints (list/role/remove members, invites, groups, custom roles) are now generally available (was beta since July 14).

**Relevance for Buildd**: If Buildd plans an enterprise workspace sync (auto-provisioning Buildd seats from Claude Enterprise org membership), the Admin API is now stable to build against.

### MCP 2026-07-28 Spec Rolling Out

The new MCP specification (finalized July 28) is rolling out across Claude products. Key changes:

- **Stateless core**: Moves from bidirectional stateful protocol to request/response — MCP servers can now deploy on serverless/edge infrastructure
- **OAuth/OIDC hardening**: `iss` validation per RFC 9207, client credentials bound to issuer, dynamic client registration deprecated in favor of Client ID Metadata Documents
- **Versioned extensions framework**: MCP Apps, Tasks, Enterprise Managed Auth, and OAuth Client Credentials are now formally governed extensions (not informal conventions)
- **Deprecations** (12-month runway): Roots, Sampling, Logging, legacy HTTP+SSE transport

**Relevance for Buildd**: Any MCP servers Buildd runs or integrates with should be audited for compatibility with the new spec — especially around auth. If using dynamic client registration for any MCP OAuth flows, plan migration to Client ID Metadata Documents before the deprecation window closes.

### Prompt Caching Security Fix (v2.1.237)

A security bug in prompt caching was fixed for sessions using a custom base URL or LLM gateway. The fix is in v2.1.237+.

**Action for Buildd**: If any Buildd worker deployments route through a custom gateway or LLM proxy, verify they are on v2.1.237+ to ensure prompt cache hits are attributed correctly and not silently doubling API costs (the Bedrock Content-Type proxy bug in v2.1.239 was also doubling billed calls).

### Memory Leak Fixed in Long Sessions (v2.1.238)

Unbounded memory growth in long interactive sessions has been fixed — subagent tool results are now released from memory after processing. Workers running many-turn sessions against large codebases were accumulating memory without bound.

**Action for Buildd**: Worker processes that run long tasks (mission workers, nightly batch tasks) should be on v2.1.238+ to benefit from this fix.

---

## Anthropic Business News (August 17–24, 2026)

### IPO: Institutional Roadshow Imminent

The public S-1 filing is expected imminently (Aug–Sep). Goldman Sachs, JPMorgan, and Morgan Stanley are leading. Nasdaq listing targeted for October 2026. At $965B valuation, the offering is expected to raise $60B+. API pricing and partnership terms are likely stable through IPO; watch for pricing or tier changes post-listing.

---

## New Ecosystem Projects (Since August 17, 2026)

| Project | Stars | Description |
|---------|-------|-------------|
| **OpenClaw** | 355K+ | 145K star jump since last scan (was 210K); now the most-starred software repository in GitHub history. Open-source autonomous AI agent running on-device via WhatsApp/Telegram. Shows massive demand for self-hosted agent infrastructure. |
| **Agent-deck** (claude-world) | Growing | Web-based command center for orchestrating multiple AI coding agents — AI task decomposition, DAG execution, real-time streaming, git finalize. Natural complement to Buildd's mission orchestration. |
| **claude-skill-antivirus** (claude-world) | Growing | Security scanner for Claude Code Skills — detects malicious patterns across 71K+ skills using 9 scanning engines. Relevant as the skills ecosystem scales. |
| **claude-world/claude-world** | Growing | Full autonomous Claude Code setup: 13 skills, 4 MCP servers, persistent memory, content pipeline, OpenClaw migration support. Example of production-grade agent configuration. |
| **claude-world/compress** | Growing | 6:1 skill/agent compression with semantic verification. Reduces prompt size for skill delivery without losing semantics. |

**Ecosystem trend this week**: The claude-world GitHub organization is building a coherent agent infrastructure suite — memory, security scanning, compression, and orchestration. This mirrors what Buildd provides at the coordination layer. The open-source versions validate the patterns; Buildd's differentiation is the task/mission coordination layer on top.

---

## Recommendations for Buildd

### This Week (August 24, 2026)

**#0 — Bump SDK to ^0.3.241 / Python to ^0.2.144**
Ten TypeScript SDK releases and nine Python SDK releases this week. v0.3.233 contains a BREAKING CHANGE (see #1). Location: `packages/core/package.json`. Effort: Trivial — but verify #1 before bumping.

**#1 — ⚠️ BREAKING: Re-add task tools to worker `allowedTools` after SDK v0.3.233 bump**
`TaskCreate`, `TaskGet`, `TaskUpdate`, `TaskList`, and `TodoWrite` are removed from the default tool surface on Opus 4.8+, Sonnet 5, Fable 5, and Mythos 5. Any Buildd worker that uses these tools (including tools for mission task management or session task tracking) must now explicitly include them. Check `packages/core/worker-runner.ts` for any tool surface construction and add `"Task*"` and `"TodoWrite"` to `allowedTools`, or set `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` in worker env. Effort: Low. Priority: **HIGH** (silent breakage after SDK bump).

**#2 — Remove Sonnet 5 pricing deadline alerts**
Sonnet 5 pricing is permanently frozen at $2/$10/MTok. Remove any billing alert, deadline copy, or role card note referencing the September 1 price increase. Add a note instead about the 1.30× tokenizer multiplier vs Sonnet 4.6 — this is the real cost factor to communicate. Location: `apps/web/src/components/HealthClient.tsx`, any role cards showing Sonnet 5 cost estimates. Effort: Low.

**#3 — Reflect 1.1× US inference premium in budget forecast**
Workers running in data-residency mode (`inference_geo: "us"`) are billed at 1.1× standard rates. The SDK's `total_cost_usd` now reflects this. Buildd's budget forecast card and `get_budget_forecast` action should account for this multiplier for workspaces with US-only inference configured. Location: `apps/web/src/lib/budget-forecast.ts` (or equivalent). Effort: Low.

**#4 — Use `PostToolUse` `classifierContext` to improve auto-mode decisions (SDK v0.3.236)**
The `PostToolUse` hook can now return `hookSpecificOutput.classifierContext` — a short host-provided note the auto-mode permission classifier reads when deciding whether to allow the next tool call. Buildd's hooks could add workspace-specific context (e.g., "this repo requires PR approval for pushes to main") to help the classifier make better decisions without user interruption. Location: `apps/runner/src/workers.ts` (hook config). Effort: Medium.

**#5 — Upgrade to v2.1.238+ for memory leak fix**
Workers running long tasks were accumulating memory without bound due to unreleased subagent tool results. Fixed in v2.1.238. Mission workers and nightly batch tasks benefit most. Effort: Trivial (version bump, already handled by #0).

**#6 — Audit prompt caching fix for gateway deployments (v2.1.237)**
If any Buildd worker routes through a custom LLM gateway or proxy, verify it is on v2.1.237+ to get the prompt-caching security fix. The Bedrock Content-Type proxy bug (v2.1.239) was silently doubling billed API calls for Bedrock workers behind proxies. Effort: Trivial (version check only).

**#7 — Audit MCP server compatibility with MCP 2026-07-28 spec**
The new MCP spec is rolling out across Claude. Key breaking risk: dynamic client registration deprecated — if any Buildd MCP OAuth flow uses it, plan migration to Client ID Metadata Documents. Check `apps/web/src/app/api/mcp/route.ts` for OAuth client registration patterns. Effort: Low (audit); Medium (if migration needed).

**#8 — Use `SDKContextUsage` from `/context` for worker context monitoring (SDK v0.3.232)**
The `/context` command now returns a structured `SDKContextUsage` payload instead of a markdown table. Workers can now programmatically check context fill percentage without parsing text. Wire this into Buildd's context usage monitoring to get clean typed data. Location: `packages/core/worker-runner.ts`. Effort: Low.

**#9 — Surface `ANTHROPIC_DEFAULT_MODEL` env var in workspace role config (CLI v2.1.236)**
A new `ANTHROPIC_DEFAULT_MODEL` env var overrides the session default model at startup. Buildd can set this in worker env construction to ensure each role's model tier is applied correctly without relying solely on the `--model` flag. Useful for workers on non-standard model tiers. Location: `packages/core/worker-runner.ts`. Effort: Low.

**#10 — Track admin API user management GA for enterprise seat sync**
The Claude Enterprise Admin API (user/group/role management) is now GA. If Buildd plans enterprise workspace auto-provisioning (sync Buildd workspace seats from Claude org membership), this is the stable API to build against. File as a product backlog item. Effort: Investigation.

### Still Relevant (From August 17, 2026)

**#11 — Use `terminal_slash_commands` to filter Remote Control command display (TS SDK v0.3.227)**
**#12 — Wire cross-session @mentions and /hold//refuse for peer worker messaging (CLI v2.1.232)** — Add `crossSessionInbound`/`dialogExpiry` to role schema
**#13 — Enable `subagent_type: "fork"` for context-aware retry workers (CLI v2.1.232 default)** — now default, evaluate in retry path
**#14 — Pass GitLab MR URLs directly to `--worktree` for GitLab workspaces (CLI v2.1.232/v2.1.233)**
**#15 — Use `claude remote-control --continue` for session resume in runner (CLI v2.1.229)**

### Still Relevant (From August 10, 2026)

**#16 — Fix Python SDK skill-name injection (v0.2.129 security fix)** — still critical
**#17 — Use `system/permission_denied` events for task timeline (TS SDK v0.3.223)**
**#18 — Add `resumeDropsTurn` to interrupt/resume flow (TS SDK v0.3.223)**
**#19 — Investigate `claude self-hosted-runner` as Buildd Enterprise deployment mode**
**#20 — Expose `sandbox.filesystem.disabled` in role configuration**
**#21 — Surface `api_error_status: 529` in task error UI**

### Still Relevant (Older)

**#22 — Use `tool_result_meta` for denied/interrupted tool classification** (SDK v0.3.216)
**#23 — Wire `cancel_queued` to interrupt endpoint** (SDK v0.3.219)
**#24 — Fix billing accuracy with `canonicalModel` + `provider`** (SDK v0.3.218)
**#25 — Surface `fast_mode_disabled_reason` in role/task UI** (SDK v0.3.219)
**#26 — Add `terminal_reason` to task completion payload** (Python v0.2.126)
**#27 — Expose subagent depth and concurrency caps in mission/role config** (SDK v0.3.217)
**#28 — Adopt `sandbox.network.strictAllowlist` for locked-down roles** (SDK v0.3.219)
**#29 — Evaluate `codebase-memory-mcp` for token reduction** (32K-star project)
**#30 — Adopt mid-conversation tool changes beta** (Fable 5/Opus 5)
**#31 — Use `agentProgressSummaries` for live task visibility** (v0.3.162+)
**#32 — OpenTelemetry worker observability**
**#33 — `SessionStore` for transcript persistence** (alpha)

---

## SDK Releases (August 10 – August 17, 2026)

6 CLI releases, 5 TypeScript SDK releases, 1 Python SDK release this week.

### TypeScript SDK v0.3.227 – v0.3.231

| Version | Key Changes |
|---------|-------------|
| **v0.3.227** | Added `terminal_slash_commands` to `system/init` message — Remote Control clients can hide terminal-oriented slash commands from their UI; conversations whose messages alone exceed the API's 32 MB limit now end with `terminal_reason: "api_error"` instead of `"image_error"`; various CLI v2.1.227 parity fixes |
| **v0.3.228** | Parity with CLI v2.1.228 (visual-freeze and Windows bug fixes) |
| **v0.3.229** | Parity with CLI v2.1.229; `remote-control --continue` resume plumbing; SSE keepalive pings for gateway streaming |
| **v0.3.230** | Parity update (CLI v2.1.230 not published to npm) |
| **v0.3.231** | Fixed MCP OAuth sign-in failing with redirect URI mismatch for pre-registered OAuth clients (Slack, etc.); parity with CLI v2.1.231 |

### Python SDK v0.2.135

- CLI bundle bump to v2.1.231; no functional changes.

### CLI v2.1.227 – v2.1.232

| Version | Key Changes |
|---------|-------------|
| **v2.1.227** (Aug 10) | Fixed critical subscription verification bug; fixed Bash command execution regression; easier slash-command menu navigation; parity base for TS SDK v0.3.227 |
| **v2.1.228** (Aug 11) | Fixed interactive sessions freezing visually while still running in background; Windows session management fixes |
| **v2.1.229** (Aug 12) | **New**: `claude remote-control --continue` — resumes the most recent Remote Control session without needing a session ID; **New**: server-supplied Claude Code hook support for self-hosted runner sessions (matches managed-environment behavior); **New**: SSE keepalive pings during long thinking pauses for gateway deployments (Vertex, Bedrock — prevents idle-timeout disconnects); **New**: plugin marketplace command sources (local IDE command prints plugin directory, re-resolved each session) |
| **v2.1.230** | Not published to npm registry |
| **v2.1.231** (Aug 13) | Fixed MCP OAuth sign-in redirect URI mismatch for pre-registered OAuth clients (Slack, GitHub, etc.) |
| **v2.1.232** (Aug 13–14) | **New**: Cross-session @name mentions, `/hold`, `/refuse` commands — interactive sessions can now reference and message peers by name; **New**: `subagent_type: "fork"` now **on by default** — fork subagents inherit full conversation and prompt cache; non-teammate agent spawns in interactive sessions run in background by default; **New**: Unique session name enforcement (duplicate names get a `name-word-word` variant); **New**: GitLab merge request URL support for `--worktree` flag and `claude agents` view (MRs shown as `!N`); **New**: Faster self-hosted runner starts; Fixed: self-hosted runners no longer fail when checkout hook fails on a repo the session doesn't push to (skipped with warning instead); **Security**: 12 security fixes including Windows permission-bypass patches; 49 total changes |

---

## New Platform Features (August 10–17, 2026)

### Text Watermarking for EU AI Act Compliance (August 11)

Anthropic announced that all Claude models released on or after **August 2, 2026** now embed invisible watermarks in generated text. Models released before August 2 will gain support over coming months.

**Technical approach**: Adaptation of Google DeepMind's SynthID-Text (Nature 2024) — changes the source of randomness used to select the next word, embedding an imperceptible signal that survives copy-paste. Files carry signed C2PA provenance information.

**Regulatory trigger**: EU AI Act Article 50, enforceable August 2, 2026 for newly launched AI systems. Anthropic applied the requirement globally (not EU-only).

**Coverage**: Claude Platform API, claude.ai, Claude Code, Claude Cowork, Claude Tag, AWS Bedrock, Google Cloud, Microsoft Foundry.

**Detection API**: Announced as forthcoming — implementation details not yet finalized.

**Relevance for Buildd**: Workers' generated code and text now carry invisible watermarks. For enterprise customers subject to EU AI Act obligations, this is a compliance feature that can be surfaced positively. For customers building AI-content detection pipelines, the upcoming detection API is worth tracking. No action required today.

### ~~Legacy Workbench Retirement~~ (August 17 — Completed Today)

See URGENT section above.

### Compliance API Expansion to Cowork and Claude Code (Enterprise Beta)

The Claude Compliance API now covers:
- **Claude Cowork**: desktop app, web, and mobile sessions
- **Claude Code**: CLI and desktop app sessions

Security and compliance teams can pull session content and metadata through the same Compliance API interface already used for Claude chats. The new endpoints are additive — no changes to existing data flows.

**Current gaps** (not yet covered): Claude Code on web, Claude Code via Claude Platform API, Bedrock sessions, Vertex AI sessions.

**Relevance for Buildd**: Enterprise Buildd workspaces could close a compliance gap here. If Buildd ever ships a compliance/audit dashboard for enterprise customers, the Compliance API (now covering Cowork + CLI Code) is the data source to integrate. Coordinate with existing `PATCH /api/workers/[id]` audit trail.

### GitLab Merge Request Support (CLI v2.1.232)

The `--worktree` flag and `claude agents` view now accept GitLab merge request URLs. MRs are displayed as `!N` notation (matching GitLab convention vs GitHub's `#N`).

**Relevance for Buildd**: Workspaces with GitLab-backed repos can now pass the MR URL directly in task context instead of constructing a branch name. Update `apps/web/src/lib/git-context.ts` (or equivalent) to detect GitLab MR URLs and pass them directly to `--worktree` when available.

### Self-Hosted Runner Improvements (CLI v2.1.229/v2.1.232)

- Faster startup times (reduced init overhead)
- Server-supplied hook support — self-hosted runner sessions now execute hooks defined by the self-hosted-runner environment, matching managed-environment behavior
- Checkout hook failure handling: if the checkout hook fails for a repository the session doesn't push to, it is now skipped with a warning instead of failing the runner

**Relevance for Buildd**: For enterprise customers evaluating `claude self-hosted-runner` as an alternative to Buildd's workers, these improvements reduce friction. Buildd's coordination and observability layer remains differentiated.

---

## Anthropic Business News (August 10–17, 2026)

### IPO: Public S-1 Expected Any Day

Confidential S-1 was filed June 1. The SEC review window (typically 30–60 days post-submission) is closing. Public S-1 filing expected August–September, with institutional roadshow starting shortly after. Nasdaq listing targeting October 2026; prediction market median is November 30. Offering expected to raise $60B+.

### Sonnet 5 Pricing Sunset: 14 Days Left

Standard pricing ($3/$15/MTok) takes effect September 1. Any Buildd workspace budget forecast, role card cost estimate, or billing alert set against the $2/$10 intro rate needs updating before August 31.

---

## New Ecosystem Projects (Since August 10, 2026)

| Project | Stars | Description |
|---------|-------|-------------|
| **cc-switch** (farion1231) | Growing | Cross-platform desktop assistant unifying Claude Code, Codex, OpenCode, Grok Build, and Hermes Agent in one UI — "switcher" for multi-agent workflows |
| **PrimeIntellect-ai/prime-agent** | Growing | Self-improving RLM agent built for coding workflows and long-running autonomous tasks |
| **TencentCloud/TencentDB-Agent-Memory** | Growing | Team-level memory hub: converts conversations, docs, and code into four reusable memory assets for AI agents |
| **OpenClaw** (sst) | 210K+ | Still the fastest-growing open-source project in ecosystem (was 188K last week); 565+ community skills, persistent memory, Google Workspace integration |

**Ecosystem trend**: Agent infrastructure tooling (memory, governance, multi-agent routing) is now outpacing raw agent frameworks in GitHub velocity. The ecosystem is maturing from demos to production deployments.

---

## Recommendations for Buildd

### This Week (August 17, 2026)

**#0 — Bump SDK to ^0.3.231 / Python to ^0.2.135**
Five TS SDK releases this week. Most are CLI parity bumps, but v0.3.227 adds `terminal_slash_commands` to system/init and corrects the `terminal_reason` for oversized conversations. Location: `packages/core/package.json`. Effort: Trivial.

**#1 — Use `terminal_slash_commands` to filter Remote Control command display (TS SDK v0.3.227)**
The `system/init` message now includes a `terminal_slash_commands` field listing commands that are terminal-only. Remote Control clients (including future Buildd mobile/web workers) should hide these commands rather than surface them as available options. Location: any Buildd component that reads `system/init` plugin/command data. Effort: Low.

**#2 — Wire cross-session @mentions and /hold//refuse for peer worker messaging (CLI v2.1.232)**
With v2.1.232, cross-session messaging has a richer protocol: `@name` addressing, `/hold` (queue message for approval), and `/refuse` (decline a held message). Buildd's `send_agent_message` action operates at the API level — consider whether peer-to-peer worker messaging (bypassing Buildd's routing) should be surfaced or restricted in the role config. Add `crossSessionInbound` and `dialogExpiry` to role schema (already recommended last week — bump to high priority). Location: `packages/shared/src/types.ts`, `apps/web/src/lib/role-config.ts`. Effort: Low.

**#3 — Enable `subagent_type: "fork"` for context-aware retry workers (CLI v2.1.232 default)**
Fork-type subagents now inherit full conversation history and prompt cache by default — making them nearly free to spawn (no cold-start context injection cost). Buildd's retry workers currently re-claim tasks with fresh context. A forked retry could resume exactly where the prior worker left off, with full context intact, at significantly lower token cost. Evaluate: modify the retry dispatch path in `packages/core/worker-runner.ts` to use `subagent_type: "fork"` when retrying a task with an existing session. Effort: Medium.

**#4 — Pass GitLab MR URLs directly to `--worktree` for GitLab workspaces (CLI v2.1.232)**
Claude Code now natively accepts GitLab MR URLs in `--worktree`. Buildd workspaces with GitLab integration can simplify task context: pass the MR URL directly instead of computing the branch name. Detect `gitlab.com/.../merge_requests/N` pattern in workspace gitConfig. Effort: Low.

**#5 — Use `claude remote-control --continue` for session resume in runner (CLI v2.1.229)**
When a Buildd worker's Remote Control session is interrupted, the runner can now resume it with `claude remote-control --continue` instead of tracking session IDs. Simplifies the reconnection flow in `apps/runner/src/workers.ts`. Effort: Low.

**#6 — Fix SSE keepalive for Vertex/Bedrock gateway deployments (CLI v2.1.229)**
If any Buildd worker deployment routes through a custom gateway on Vertex or Bedrock, v2.1.229's SSE keepalive pings prevent idle-timeout disconnects during long thinking pauses. Verify all production worker deployments are on v2.1.229+. Effort: Trivial (version check).

**#7 — ⚠️ FINAL CALL: Update Sonnet 5 billing before August 31**
14 days remain. Sonnet 5 pricing reverts from $2/$10 to $3/$15/MTok on September 1. Update `apps/web/src/components/HealthClient.tsx` (budget forecast card), role cards showing Sonnet 5 cost estimates, and any workspace billing alert thresholds. Effort: Low. Deadline: **August 31**.

**#8 — Track Compliance API beta for enterprise audit dashboard (new)**
The Compliance API now covers Cowork and Claude Code (CLI + desktop) for Enterprise customers. If Buildd plans an enterprise audit/compliance view, this is the API to integrate. File as a product backlog item; no immediate engineering action. Effort: Investigation.

**#9 — Surface text watermarking as enterprise compliance feature**
Claude-generated text now carries invisible watermarks per EU AI Act Article 50. For enterprise Buildd customers under EU regulation, this is a compliance story to surface in the product. Add a note to the role config UI or workspace settings: "AI-generated text from workers meets EU AI Act Article 50 watermarking requirements." When Anthropic's detection API launches, consider adding a verification endpoint. Effort: Low (copy/UI only until detection API available).

### Still Relevant (From August 10, 2026)

**#10 — Fix Python SDK skill-name injection (v0.2.129 security fix)** — still critical
**#11 — Use `system/permission_denied` events for task timeline (TS SDK v0.3.223)**
**#12 — Add `resumeDropsTurn` to interrupt/resume flow (TS SDK v0.3.223)**
**#13 — Investigate `claude self-hosted-runner` as Buildd Enterprise deployment mode**
**#14 — Add `crossSessionInbound`/`dialogExpiry` to role config schema** (now #2 above — promoted)
**#15 — Expose `sandbox.filesystem.disabled` in role configuration**
**#16 — Surface `api_error_status: 529` in task error UI**
**#17 — Add archive-source plugin install to skill setup**

### Still Relevant (Older)

**#18 — Use `tool_result_meta` for denied/interrupted tool classification** (SDK v0.3.216)
**#19 — Wire `cancel_queued` to interrupt endpoint** (SDK v0.3.219)
**#20 — Fix billing accuracy with `canonicalModel` + `provider`** (SDK v0.3.218)
**#21 — Surface `fast_mode_disabled_reason` in role/task UI** (SDK v0.3.219)
**#22 — Add `terminal_reason` to task completion payload** (Python v0.2.126)
**#23 — Expose subagent depth and concurrency caps in mission/role config** (SDK v0.3.217)
**#24 — Adopt `sandbox.network.strictAllowlist` for locked-down roles** (SDK v0.3.219)
**#25 — Evaluate `codebase-memory-mcp` for token reduction** (32K-star project)
**#26 — Adopt mid-conversation tool changes beta** (Fable 5/Opus 5)
**#27 — Use `agentProgressSummaries` for live task visibility** (v0.3.162+)
**#28 — OpenTelemetry worker observability**
**#29 — `SessionStore` for transcript persistence** (alpha)
**#30 — Dynamic Workflows compatibility decision** (unbounded — cap removed)
...
|------|-------------------|-------------------|-------------|-------------|
| 2026-08-17 | 0.3.227-0.3.231 | 0.2.135 | 2.1.227-2.1.232 | **Legacy Workbench retired today** + experimental prompt APIs (generate/improve/templatize) gone permanently; **text watermarking** GA (EU AI Act Article 50, SynthID-Text-style, Aug 2+ models globally, detection API coming); **Compliance API** expanded to Cowork + Claude Code CLI/desktop (Enterprise beta); **cross-session @mentions, /hold, /refuse** (v2.1.232); **`subagent_type: "fork"` on by default** (inherits full conv + prompt cache); **GitLab MR URL** support for `--worktree` + agents view; `claude remote-control --continue` for session resume; SSE keepalive pings for Vertex/Bedrock gateways; server-supplied hooks for self-hosted runners; MCP OAuth redirect-URI fix; `terminal_slash_commands` in system/init; OpenClaw 210K+ stars; cc-switch multi-agent switcher |
