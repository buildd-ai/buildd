...
export function buildParamsDescription(actions: readonly string[]): string {
  const descriptions: Record<string, string> = {
    ...
    create_task: '{ title (required), description (required), workspaceId?, priority?, category? (bug|feature|refactor|chore|docs|test|infra|design — auto-detected if omitted), startAt? (future ISO 8601), startIn? (45m|3h|2d), startAfter? ("budget_reset"; mutually exclusive with startAt/startIn), outputRequirement? (pr_required|artifact_required|none|auto — default auto), outputSchema?, project? (monorepo project name for scoping), missionId? (auto-inherited from caller), parentTaskId? (link retry to original task), dependsOn? (array of task IDs that must complete AND have their PRs merged before this task is claimable — REQUIRED for acceptance/gate/validation tasks), pathManifest? (array of file paths/globs this task will create or modify; overlapping sibling manifests auto-add dependency edges), roleSlug? (route to specific role), baseBranch?, verificationCommand?, iteration?, maxIterations?, failureContext?, skillSlugs?, tier? (premium|standard|budget), model? (full model ID; bypasses tier resolution), effort? (low|medium|high), callbackUrl?, callbackToken?, release? ("true"|"false"|"inherit"), backend? (claude|codex) } — Pre-filed chain: create the mission held + manual, create each task with missionId plus dependsOn/pathManifest, set orchestrationMode="auto" only if organizer coordination is wanted, then arm it. A task inherits hold and pacing gates from its mission, so pending is intentional until those gates clear. Deferred tasks likewise wait for resolved startAt. Unknown parameters are rejected.',
    ...
    manage_missions: '{ action: "list" | "create" | "get" | "update" | "arm" | "delete" | "link_task" | "unlink_task", missionId?, title?, description?, workspaceId?, initiativeId? (parent initiative; null unlinks), cronExpression?, priority?, status?, taskId?, startAt? (future ISO 8601), startIn? (45m|3h|2d), startAfter? ("budget_reset"), skillSlugs?, model?, isHeartbeat?: boolean, heartbeatChecklist?: string, activeHoursStart?: number, activeHoursEnd?: number, activeHoursTimezone?: string, maxConcurrentTasks?: number (mission claim cap), dependsOnMission?: string, gateCondition?: "merged" | "completed", costBudgetUsd?: number, startMode?: "armed" | "held", orchestrationMode?: "auto" | "manual", pacingMode?: "eager" | "paced", pacingMaxPerHour?: number } — Choose independently: startMode default: "armed"; use "held" while filing or reviewing a chain—"held" blocks normal linked task claims until action="arm" (a force-start can bypass one task; held otherwise means intentional waiting, not broken), but held does not suppress organizer evaluation. orchestrationMode default: "auto"; "auto" runs the organizer, but pre-filed tasks present at its first evaluation make it skip decomposition and coordinate only; "manual" skips automatic organizer initiative while linked tasks still run through other gates, although explicit Run now can still invoke it. pacingMode default: "eager" (start whenever other gates allow); "paced" spaces task starts by 3600/pacingMaxPerHour seconds, and pacingMaxPerHour default: 1. A paced pending task is waiting for its next start slot, not stuck. Deferred missions wait for resolved startAt. [admin]',
