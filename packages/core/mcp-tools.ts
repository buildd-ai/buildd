...
export function buildParamsDescription(actions: readonly string[]): string {
  const descriptions: Record<string, string> = {
    ...
    manage_initiatives: '{ action: "list" | "create" | "get" | "update" | "delete" | "link_mission" | "unlink_mission", initiativeId?, missionId? (for link/unlink), title?, description?, workspaceId?, status?: "planned" | "active" | "paused" | "completed" | "archived" (set by a person; nothing derives or auto-advances it), priority?: number, ownerUserId?: string (a member of the initiative\'s team; null falls back to the creator; create defaults to the caller), targetDate?: "YYYY-MM-DD" | null (optional calendar target). DEPRECATED and slated for removal: kpis, autoVerify, action=evaluate, action=get_kpi_state (still accepted, no longer shown on the dashboard; use mission goalCriteria for anything buildd can check). } — an initiative is an execution-free container above missions (initiative → mission → task), like a Linear initiative. Progress is missions done over missions. "get" returns a KB-optimized brief: rolled-up progress + child missions + initiative-level artifacts. Create/update auto-index the initiative into the team knowledge base (recall/query_knowledge corpus=initiative). [admin]',
...
export async function handleBuilddAction(
  ...
  switch (action) {
    ...
    case 'close_pr': {
      ...
      const data = await api('/api/github/pr', {
        method: 'PATCH',
        body: JSON.stringify({ workerId, prNumber: params.prNumber }),
      });
    ...
    case 'manage_initiatives': {
      ...
      switch (initiativeAction) {
        ...
        case 'create': {
          ...
          // DEPRECATED: kpis / autoVerify pass through until the columns are dropped.
        ...
        case 'get': {
          ...
          // DEPRECATED: kpis + autoVerify + last KPI evaluation state, shown while
          // the fields still exist so an agent that set them can read them back.
        ...
        // DEPRECATED: evaluate / get_kpi_state go with initiative KPIs.
        case 'evaluate': {
          if (!params.initiativeId) throw new Error('initiativeId is required');
